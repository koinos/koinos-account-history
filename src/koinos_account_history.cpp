...
void log_func( const boost::system::error_code& ec,
...
void sync_func( const boost::system::error_code& ec,
                std::shared_ptr< boost::asio::system_timer > timer,
                std::shared_ptr< account_history::account_history > ah,
                ...
                std::shared_ptr< std::atomic< uint64_t > > chain_lib_ptr )
{
  ...
  if( ah_lib < chain_lib )
  {
    ...
    auto future = client->rpc( util::service::chain,
                               util::converter::as< std::string >( chain_req ),
                               1'000ms,
    ...
    if( !chain_resp.has_error() && chain_resp.has_get_head_info() )
    {
      uint64_t request_size =
        std::min( account_history::max_request_limit, chain_resp.get_head_info().head_topology().height() - ah_lib );

      rpc::block_store::block_store_request bs_req;
      bs_req.mutable_get_blocks_by_height()->set_allocated_head_block_id(
        chain_resp.mutable_get_head_info()->mutable_head_topology()->release_id() );
      ...
      auto future = client->rpc( util::service::block_store,
                                 util::converter::as< std::string >( bs_req ),
                                 1'000ms,
                                 mq::retry_policy::exponential_backoff );
      ...
      if( !bs_resp.has_error() && bs_resp.has_get_blocks_by_height() )
      {
        ...
        for( uint32_t i = 0; i < blocks_by_height->block_items_size(); i++ )
        {
          ...
          if( request_size == account_history::max_request_limit
              && block_accept.block().header().height() % 1'000 == 0 )
          {
            auto to_go = std::chrono::duration_cast< std::chrono::seconds >(
              std::chrono::system_clock::now().time_since_epoch()
              - std::chrono::milliseconds( block_accept.block().header().timestamp() ) );
            LOG( info ) << "Sync progress - Height: " << block_accept.block().header().height()
                        << ", ID: " << util::to_hex( block_accept.block().id() ) << " ("
                        << util::format_duration( to_go ) << " block time remaining)";
          }
          ...
          if( block_accept.block().header().height() <= chain_lib )
          {
            ...
            block_irr.mutable_topology()->set_allocated_previous(
              block_accept.mutable_block()->mutable_header()->release_previous() );
        ...
        timer->async_wait(
          boost::bind( &sync_func, boost::asio::placeholders::error, timer, ah, client, chain_lib_ptr ) );
        return;
      }
    }
  }
...
int main( int argc, char** argv )
{
  ...
  try
  {
    ...
    auto amqp_url =
      util::get_option< std::string >( AMQP_OPTION, AMQP_DEFAULT, args, account_history_config, global_config );
    auto log_level = util::get_option< std::string >( LOG_LEVEL_OPTION,
                                                      LOG_LEVEL_DEFAULT,
                                                      args,
                                                      account_history_config,
                                                      global_config );
    auto log_dir =
      util::get_option< std::string >( LOG_DIR_OPTION, LOG_DIR_DEFAULT, args, account_history_config, global_config );
    auto log_color =
      util::get_option< bool >( LOG_COLOR_OPTION, LOG_COLOR_DEFAULT, args, account_history_config, global_config );
    auto log_datetime = util::get_option< bool >( LOG_DATETIME_OPTION,
                                                  LOG_DATETIME_DEFAULT,
                                                  args,
                                                  account_history_config,
                                                  global_config );
    ...
    auto jobs         = util::get_option< uint64_t >( JOBS_OPTION,
                                              ...
                                              account_history_config,
                                              global_config );
    auto statedir     = std::filesystem::path( util::get_option< std::string >( STATEDIR_OPTION,
                                                                            STATEDIR_DEFAULT,
                                                                            args,
                                                                            account_history_config,
                                                                            global_config ) );
    auto reset        = util::get_option< bool >( RESET_OPTION, false, args, account_history_config, global_config );
    auto fork_algorithm_option = util::get_option< std::string >( FORK_ALGORITHM_OPTION,
                                                                  FORK_ALGORITHM_DEFAULT,
                                                                  args,
                                                                  account_history_config,
                                                                  global_config );
    auto whitelist_addresses =
      util::get_options< std::string >( ADDRESS_WHITELIST_OPTION, args, account_history_config, global_config );
    ...
    koinos::initialize_logging( util::service::account_history,
                                instance_id,
                                log_level,
                                logdir_path,
                                log_color,
    ...
    std::shared_ptr< koinos::account_history::account_history > account_history =
      std::make_shared< koinos::account_history::account_history >( whitelist );
          ...
          try
          {
            switch( args.request_case() )
            {
              case rpc::account_history::account_history_request::RequestCase::kGetAccountHistory:
                {
                  *resp.mutable_get_account_history() =
                    account_history->get_account_history( args.get_account_history() );
                  break;
                }
    ...
    request_handler.add_broadcast_handler( "koinos.block.irreversible",
                                           [ & ]( const std::string& msg )
                                           {
    ...
    sync_timer->async_wait(
      boost::bind( sync_func, boost::asio::placeholders::error, sync_timer, account_history, client, chain_lib ) );
